...
abstract class CommonAsyncTask extends AsyncTask<Void, Void, Boolean> {
  ...
  final com.undrowned.WhatToDo.WhatToDo activity;
  final com.google.api.services.tasks.Tasks client;
  private final View progressBar;

  CommonAsyncTask(WhatToDo activity) {
  ...
  protected final Boolean doInBackground(Void... ignored) {
    try {
      ...
      activity.startActivityForResult(
          userRecoverableException.getIntent(), WhatToDo.REQUEST_AUTHORIZATION);
    } catch (IOException e) {
      Utils.logAndShow(activity, WhatToDo.TAG, e);
    }
    return false;
  }
